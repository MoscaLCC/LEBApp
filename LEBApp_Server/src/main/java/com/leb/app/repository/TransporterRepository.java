...
public interface TransporterRepository extends JpaRepository<Transporter, Long>, JpaSpecificationExecutor<Transporter> {
    @Query(
        value = "select distinct transporter from Transporter transporter left join fetch transporter.ridePaths",
    ...
    @Query("select distinct transporter from Transporter transporter left join fetch transporter.ridePaths")
