...
import com.leb.app.repository.ZoneRepository;
import com.leb.app.service.ZoneService;
import com.leb.app.service.criteria.ZoneCriteria;
import com.leb.app.service.dto.ZoneDTO;
import com.leb.app.service.mapper.ZoneMapper;
...
class ZoneResourceIT {
    ...
    public static Zone createEntity(EntityManager em) {
        Zone zone = new Zone().name(DEFAULT_NAME);
        return zone;
    }
    ...
    public static Zone createUpdatedEntity(EntityManager em) {
        Zone zone = new Zone().name(UPDATED_NAME);
        return zone;
    }
    ...
    @Test
    @Transactional
    void getAllZonesByPointIsEqualToSomething() throws Exception {
        // Initialize the database
        zoneRepository.saveAndFlush(zone);
        ...
        // Get all the zoneList where point equals to pointId
        defaultZoneShouldBeFound("pointId.equals=" + pointId);

        // Get all the zoneList where point equals to (pointId + 1)
        defaultZoneShouldNotBeFound("pointId.equals=" + (pointId + 1));
    }

    @Test
    @Transactional
    void getAllZonesByTransportersIsEqualToSomething() throws Exception {
        // Initialize the database
        zoneRepository.saveAndFlush(zone);
        Transporter transporters = TransporterResourceIT.createEntity(em);
        em.persist(transporters);
        em.flush();
        zone.addTransporters(transporters);
        zoneRepository.saveAndFlush(zone);
        Long transportersId = transporters.getId();

        // Get all the zoneList where transporters equals to transportersId
        defaultZoneShouldBeFound("transportersId.equals=" + transportersId);

        // Get all the zoneList where transporters equals to (transportersId + 1)
        defaultZoneShouldNotBeFound("transportersId.equals=" + (transportersId + 1));
    }
