...
class RequestResourceIT {
    ...
    public static Request createEntity(EntityManager em) {
        ...
        // Add required entity
        RidePath ridePath;
        if (TestUtil.findAll(em, RidePath.class).isEmpty()) {
            ridePath = RidePathResourceIT.createEntity(em);
            em.persist(ridePath);
            em.flush();
        } else {
            ridePath = TestUtil.findAll(em, RidePath.class).get(0);
        }
        request.setRidePath(ridePath);
        // Add required entity
        Producer producer;
    ...
    public static Request createUpdatedEntity(EntityManager em) {
        ...
        RidePath ridePath;
        if (TestUtil.findAll(em, RidePath.class).isEmpty()) {
            ridePath = RidePathResourceIT.createUpdatedEntity(em);
            em.persist(ridePath);
            em.flush();
        } else {
            ridePath = TestUtil.findAll(em, RidePath.class).get(0);
    ...
    @Test
    @Transactional
    void getAllRequestsByRidePathIsEqualToSomething() throws Exception {
        // Initialize the database
        requestRepository.saveAndFlush(request);
        ...
        // Get all the requestList where ridePath equals to ridePathId
        defaultRequestShouldBeFound("ridePathId.equals=" + ridePathId);

        // Get all the requestList where ridePath equals to (ridePathId + 1)
        defaultRequestShouldNotBeFound("ridePathId.equals=" + (ridePathId + 1));
    }
