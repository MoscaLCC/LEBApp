...
class RidePathResourceIT {
    ...
    @Test
    @Transactional
    void getAllRidePathsByRequestIsEqualToSomething() throws Exception {
        // Initialize the database
        ridePathRepository.saveAndFlush(ridePath);
        Request request = RequestResourceIT.createEntity(em);
        em.persist(request);
        em.flush();
        ridePath.addRequest(request);
        ridePathRepository.saveAndFlush(ridePath);
        Long requestId = request.getId();

        // Get all the ridePathList where request equals to requestId
        defaultRidePathShouldBeFound("requestId.equals=" + requestId);

        // Get all the ridePathList where request equals to (requestId + 1)
        defaultRidePathShouldNotBeFound("requestId.equals=" + (requestId + 1));
    }

    @Test
    @Transactional
    void getAllRidePathsByTransportsIsEqualToSomething() throws Exception {
        // Initialize the database
        ridePathRepository.saveAndFlush(ridePath);
        Transporter transports = TransporterResourceIT.createEntity(em);
        em.persist(transports);
        em.flush();
        ridePath.addTransports(transports);
        ridePathRepository.saveAndFlush(ridePath);
        Long transportsId = transports.getId();

        // Get all the ridePathList where transports equals to transportsId
        defaultRidePathShouldBeFound("transportsId.equals=" + transportsId);

        // Get all the ridePathList where transports equals to (transportsId + 1)
        defaultRidePathShouldNotBeFound("transportsId.equals=" + (transportsId + 1));
    }
    ...
    void partialUpdateRidePathWithPatch() throws Exception {
        ...
        partialUpdatedRidePath.destination(UPDATED_DESTINATION);
